...
export const Discord = () => {
    ...
    if (!presence)
        return <div className="text-sm text-gray-400">Connecting to Discord...</div>;
    ...
    const getDiscordAssetUrl = (
        ...
        if (imageName.startsWith("mp:external/")) {
            return `https://media.discordapp.net/${imageName.replace("mp:external/", "")}`;
        }

        const cleanName = imageName.replace(/^(large_|small_)/, "").replace(/\?.*$/, "");
        return `https://cdn.discordapp.com/app-assets/${appId}/${cleanName}.png`;
    };

    const getDynamicFallback = (activityName?: string): string | null => {
        if (!activityName) return null;
        const key = activityName.toLowerCase().replace(/\s+/g, "_");
        return `https://cdn.jsdelivr.net/gh/Azn9/Jetbrains-Discord-Integration@develop/icons/data/themes/material/applications/${key}.png`;
    };

    const largeImageUrl = getDiscordAssetUrl(app?.application_id, app?.assets?.large_image);
    const smallImageUrl = getDiscordAssetUrl(app?.application_id, app?.assets?.small_image);

    return (
        <div className={`p-4 rounded-2xl shadow-lg w-full max-w-md mx-auto m-4 ${containerClass}`}>
                        ...
                        <div className="flex items-center gap-3 mt-3">
                            {largeImageUrl && (
                                <img
                                    ...
                                    onError={(e) => {
                                        const fallback = getDynamicFallback(app.name);
